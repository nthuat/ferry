...
class ModelScope(
    ...
    override suspend fun manifest(repoId: String): Result<RepoManifest> = withContext(Dispatchers.IO) {
        try {
            ...
            // ModelScopeTest against the actual request produced, not just argued for here.
            // HuggingFace originally built its URL by string interpolation and had to reject those
            // characters up front with a denylist for exactly this reason; it has since converted to
            // this same typed-API approach and dropped that denylist (see HuggingFace.manifest's own
            // comment for that history), rather than this file duplicating an equivalent guard.
            //
            ...
            // did, popping the segment before it. No denylist reasoning applies to it either way, so
            // deferring to "the hub decides which ids are valid" was a non sequitur here, not a
            // narrower case of the same argument. requireWithinNamespace below checks the *built URL*
            // instead, after this call has already decided what it meant to request: it says nothing
            // about which repo ids are legal, only that this request still lands under this adapter's
            // own models namespace, so it can't go stale the way a denylist can. See
            ...
            val listingBuilder = URLBuilder(namespace)
                .appendPathSegmentsResolvingDots(repoId)
                .appendPathSegments("repo", "files")
            listingBuilder.parameters.append("Revision", revision)
            ...
            listingBuilder.parameters.append("Recursive", "True")
            val listingUrl = listingBuilder.build()
                .also { requireWithinNamespace(it, namespace.segments, "repoId '$repoId'") }

            val response = client.get(listingUrl.toString())
            ...
            // the transport-level isSuccess check above rather than folded into it.
            if (!parsed.success) {
                return@withContext Result.failure(
                    IOException(
                        parsed.message
     ...
     * path, this hub does not. Built with [URLBuilder]'s `parameters.append`, which percent-encodes
     ...
     * a query parameter, which treats it as an opaque value to percent-encode, never as path
     ...
     * the built URL's segments stay exactly `api/v1/models/{repoId}/repo`, regardless of what [path]
     * contains. `HuggingFace` puts its per-file path in the URL path instead, which is what exposes
     * it there.
     *
    ...
    private fun downloadUrl(base: Url, repoId: String, path: String): String {
        val namespace = modelsNamespace(base)
        val builder = URLBuilder(namespace)
            .appendPathSegmentsResolvingDots(repoId)
            .appendPathSegments("repo")
        builder.parameters.append("Revision", revision)
        builder.parameters.append("FilePath", path)
        val full = builder.build()
        requireWithinNamespace(full, namespace.segments, "repoId '$repoId'")
     ...
     */
    private fun modelsNamespace(base: Url): Url = URLBuilder(base)
        .appendPathSegments("api", "v1", "models")
        .build()
