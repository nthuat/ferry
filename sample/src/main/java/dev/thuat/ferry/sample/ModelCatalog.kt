 ...
 *
 * [fileCount] and [sizeLabel] are shown only in the [dev.thuat.ferry.sample.DownloadState.Available]
 * row, before any network call. The moment a real check happens — a `WontFit` refusal, a completed
 * download — every number shown instead comes from a live [dev.thuat.ferry.SpaceReport] or
 * [dev.thuat.ferry.RepoProgress], never from here, so this catalog can never disagree with
