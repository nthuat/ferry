...
> interrupted download resumes across process death. Pause is the one thing still not done.
> On Maven Central as `dev.thuat:ferry:0.1.0`.

**Contents:** [Getting it](#getting-it) ·
[Why this exists](#why-this-exists) ·
[Guarantees](#guarantees) ·
[HuggingFace](#three-things-about-huggingface-worth-knowing) ·
[Range requests](#two-things-about-http-range-requests-worth-knowing) ·
[Adding a hub](#adding-a-hub) ·
[Sample app](#sample-app) ·
[Backgrounding](#backgrounding-ferry-work) ·
[Building](#building)

**0.x note:** `RepoProgress` is a sealed interface, and pause is still unimplemented. Pause means
stopping a download on purpose and recording that, rather than recording it as a failure (see
Guarantee 4). Adding it later needs a new `RepoProgress` case, which breaks any consumer's exhaustive
`when`. The hub interface (`ModelHub`) is likewise expected to grow a real error taxonomy beyond a
bare `IOException`. Both may break before `1.0.0`. That is a deliberate use of what `0.x` means in
semver, not instability.
...
travels through your interceptors, your timeouts and your proxy config, rather than through a second
...
`<uses-permission android:name="android.permission.INTERNET" />`, because Ferry no longer ships a
manifest to carry it for you. (`:ferry-work` still declares it, since it depends on `:ferry` and does
the network call.)
...
### What Guarantee 4 actually covers

It covers more ground than "resumable" usually means, and what stays out of scope is the honest part.

- A dropped connection is recovered from mid-attempt via `Range`.
- A file already staged and verifying, from this attempt or an earlier one, is not fetched again.
- Staging itself is durable. A failed attempt, a cancellation, or the process dying all leave it
  exactly as far as it got. Nothing deletes it until either a later `download` call consumes it by
  committing the repo, or the caller explicitly calls `abandonStaging` because no retry is coming.
- A second `download` call for the same repo id, even from a fresh process, resumes from whatever is
  already on disk instead of restarting from zero.
- `stagedBytes(repoId, into)` lets a caller detect there is something to resume before calling
  `download` at all. That is the number behind a "Resume, N already downloaded" row.

Two things remain out of scope.

**Deliberate pause.** Stopping a download mid-flight and recording that it was intentional rather
than a failure needs cooperative cancellation threaded through the transfer loop, which nothing here
does yet. Coroutine cancellation already stops a transfer, but nothing records why it stopped.

**Resume is only ever as good as the hub's own validator.** A `.part` with no `ETag` or
`Last-Modified` from the server restarts that one file from byte zero, rather than risk resuming onto
...
progress bar cannot show a guarantee; only the absence of one is visible.

It lists three real HuggingFace repos (a 4.7 MB, an 18 MB, and the 5.6 GB `gpt2`) plus a sabotage
panel with two controls, both built on seams the library already exposes:

- One pretends the disk is nearly full, via a `FreeSpaceProbe` passed into a directly-constructed
  `RepoDownloader`, so every model's `WontFit` row shows real `SpaceReport` numbers.
- The other flips a byte in a downloaded file on disk, so tapping Re-check visibly redownloads it
  instead of trusting what is there.

Neither adds anything to `:ferry`.
...
The reason is this README's own "Why this exists" table. Alibaba's MNN backgrounds downloads with a
...
`:ferry` is a plain JVM module and needs only a JDK. Building the rest of the repo (`:ferry-work` and
the `:sample` app) additionally requires an Android SDK with API 35, and a JDK between 17 and 21.
...
Four credentials drive publication. All are read from a Gradle property, or from an environment
variable of the same name in SCREAMING_SNAKE_CASE (`gradle/publishing.gradle.kts`), and none has a
default:
...
With them set, a release is four steps: tag the commit, deploy, promote, publish.
